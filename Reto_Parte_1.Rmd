...
```
*Relación analítica en el máximo*:Para encontrar la relación entre los parámetros en el máximo de la curva de infección ($I$), analizamos la ecuación diferencial de $dI/dt$:$$ \frac{dI}{dt} = \beta \frac{I}{N} S - \gamma I $$En el punto máximo (el pico), la pendiente es cero ($dI/dt = 0$). Entonces:$$ 0 = I \left( \beta \frac{S}{N} - \gamma \right) $$Como $I \neq 0$, debemos tener:$$ \beta \frac{S}{N} = \gamma \implies S = \frac{\gamma}{\beta} N $$O equivalentemente, usando el número reproductivo básico $R_0 = \beta / \gamma$:$$ S_{pico} = \frac{N}{R_0} $$Esto significa que el pico ocurre exactamente cuando la población susceptible $S$ cae hasta alcanzar el umbral $N/R_0$.
